...
class HPComware512900Base(CiscoSSHConnection):
    ...
    def config_mode(self, config_command='system-view'):
        ...
        return super(HPComware512900Base, self).config_mode(config_command=config_command)

    def exit_config_mode(self, exit_config='return', pattern=r'>'):
        """Exit config mode."""
        return super(HPComware512900aBase, self).exit_config_mode(exit_config=exit_config,
                                                           pattern=pattern)

    def check_config_mode(self, check_string=']'):
        """Check whether device is in configuration mode. Return a boolean."""
        return super(HPComware512900Base, self).check_config_mode(check_string=check_string)
    ...
    def set_base_prompt(self, pri_prompt_terminator='>', alt_prompt_terminator=']',
        ...
        """
        prompt = super(HPComware512900Base, self).set_base_prompt(
    ...
    def save_config(self, cmd='save force', confirm=False):
        """Save Config."""
        return super(HPComware512900Base, self).save_config(cmd=cmd, confirm=confirm)


class HPComware512900SSH(HPComware512900Base):
    pass


class HPComware512900Telnet(HPComware512900Base):
    def __init__(self, *args, **kwargs):
        default_enter = kwargs.get('default_enter')
        kwargs['default_enter'] = '\r\n' if default_enter is None else default_enter
        super(HPComware512900Telnet, self).__init__(*args, **kwargs)
